...
public class SwerveSubsystem extends SubsystemBase {
  ...
  public Rotation2d getTargetAutoAimRotation() {
    Pose2d pose = getPose();
    ...
    if (robotX < 5.5) {
      ...
      Logger.recordOutput("Distance to Hub", dist);
    ...
    Logger.recordOutput("Distance to Hub", -1.0);
    return null;
  }
  ...
  public SwerveSubsystem(File directory) {
    ...
    try {
      ...
      pigeon.getPitch().setUpdateFrequency(10);
      pigeon.getRoll().setUpdateFrequency(10);
      pigeon.getQuatW().setUpdateFrequency(10);
    } catch (Exception e) {
    ...
    visionNotifier = new Notifier(() -> {
      if (Constants.OperatorConstants.VISION_ENABLED) {
  ...
  public void periodic() {
    Logger.recordOutput("Subsystems/Drive/Robot X", swerveDrive.getPose().getX());
    Logger.recordOutput("Subsystems/Drive/Robot Y", swerveDrive.getPose().getY());
    Logger.recordOutput("Subsystems/Drive/Robot Heading", swerveDrive.getPose().getRotation().getDegrees());

    double yawRate = pigeon.getAngularVelocityZDevice().getValueAsDouble();
    double pitch = getPitch().getDegrees();
    double roll = swerveDrive.getRoll().getDegrees();

    Logger.recordOutput("Subsystems/Drive/IMU Pitch", pitch);
    Logger.recordOutput("Subsystems/Drive/IMU Roll", roll);
    Logger.recordOutput("Subsystems/Drive/IMU Yaw Rate", yawRate);
    Logger.recordOutput("Subsystems/Drive/Max Speed", swerveDrive.getMaximumChassisVelocity());
